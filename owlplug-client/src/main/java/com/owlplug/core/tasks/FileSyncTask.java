...
public class FileSyncTask extends AbstractTask {
  ...
  protected TaskResult call() throws Exception {
    ...
    try {
      ...
      length = extractFolderSize(directory);
    } catch (Exception e) {
  ...
  public long extractFolderSize(File directory) {
    long length = 0;

    this.updateMessage("Running file sync on directory: " + directory.getAbsolutePath());
    fileStatDAO.deleteByPath(directory.getAbsolutePath());

    FileStat directoryStat = new FileStat();
    directoryStat.setName(directory.getName());
    directoryStat.setPath(directory.getAbsolutePath());
    directoryStat.setParentPath(directory.getAbsolutePath());
    directoryStat.setLength(0);
    fileStatDAO.save(directoryStat);

    // TODO, set directorStat parent object

    for (File file : directory.listFiles()) {
      if (file.isFile()) {
        FileStat fileStat = new FileStat();
        fileStat.setName(file.getName());
        fileStat.setPath(file.getAbsolutePath());
        fileStat.setParentPath(directory.getAbsolutePath());
